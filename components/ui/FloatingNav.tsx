  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        transition={{ duration: 0.2 }}
        className={cn(
          "flex max-w-fit fixed bottom-10 inset-x-0 mx-auto border rounded-full shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-10 py-5 items-center justify-center space-x-4 border-white/[0.2] bg-black-100",
          className
        )}
      >
        {navItems.map((navItem, idx) => (
...
export default function Page() {
  ...
  const navItemsBottom: NavItem[] = [
    { name: "Services", link: "#services", icon: <IconHome className="h-4 w-4" /> },
    { name: "Blog", link: "#blog", icon: <IconUser className="h-4 w-4" /> },
    { name: "Help", link: "#help", icon: <IconMessage className="h-4 w-4" /> },
  ];
